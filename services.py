...
def validate_countries():
    ...
    try:
        ...
        numeric_cols = [
            ...
            'Addicted_Score', 'Affects_Academic_Performance', 'Conflicts_Over_Social_Media'
        ]
        if not all(col in df.columns for col in numeric_cols):
            return jsonify({"error": f"Columnas numéricas requeridas no encontradas: {', '.join(numeric_cols)}"}), 400

        country_means = df.groupby('Country')[numeric_cols].mean().reset_index()
        scaler = StandardScaler()
        X = scaler.fit_transform(country_means[numeric_cols])
        kmeans = KMeans(n_clusters=3, random_state=42)
        country_means['Cluster'] = kmeans.fit_predict(X)

        plots_dir = os.path.join(os.path.dirname(__file__), 'static', 'plots')
        os.makedirs(plots_dir, exist_ok=True)
        cluster_path = os.path.join(plots_dir, 'country_clusters.png')
        ...
        try:
            ...
            plt.title('Clustering de Países por Uso de Redes Sociales y Salud Mental', fontsize=14, pad=20)
            plt.xlabel('Uso Promedio de Redes Sociales (horas)')
            plt.ylabel('Puntuación Promedio de Salud Mental')
            plt.legend(title='Cluster')
            plt.savefig(cluster_path, dpi=100, bbox_inches='tight')
            plt.close()
        except Exception as e:
            logger.error(f"Error al generar el gráfico de clusters: {str(e)}")
            return jsonify({"error": f"Error al generar el gráfico de clusters: {str(e)}"}), 500

        if not os.path.exists(cluster_path) or os.path.getsize(cluster_path) == 0:
            logger.error(f"El archivo {cluster_path} no se generó correctamente o está vacío")
            return jsonify({"error": f"El archivo del gráfico de clusters no se generó correctamente"}), 500

        logger.info(f"Gráfico de clusters generado correctamente en {cluster_path}")
        cluster_assignments = country_means[['Country', 'Cluster']].set_index('Country')['Cluster'].to_dict()
        ...
        return {
            "data": {
                "countries": {
                    ...
                    "cluster_image": "/static/plots/country_clusters.png",
